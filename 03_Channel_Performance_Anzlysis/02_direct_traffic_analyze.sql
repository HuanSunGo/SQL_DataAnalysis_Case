-- Task: Analyzing Direct Traffic Channel (Trend by Month)
USE mavenfuzzyfactory;

-- have a basic understanding of paid, nonpaid channel
CREATE TEMPORARY TABLE channel_group
SELECT DISTINCT 
	CASE 
		-- first one : came from search engine, but doesn't have tracking parameter 
		WHEN utm_source IS NULL AND http_referer IN ('https://www.gsearch.com','https://bsearch.com') THEN 'organic_search' 
...
-- Result: not only the direct, organic and brand volumns are growing, but they are growing as a percentage of the paid traffic volume.
